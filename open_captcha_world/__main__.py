...
def serve_captcha(captcha_type, filename):
    return send_from_directory(os.path.join(BASE_DIR, 'captcha_data', captcha_type), filename)

@app.route('/captcha_data/<captcha_type>/<subdir>/<filename>')
def serve_captcha_subdir(captcha_type, subdir, filename):
    return send_from_directory(os.path.join(BASE_DIR, 'captcha_data', captcha_type, subdir), filename)
...
def record_benchmark():
    ...
    with open(os.path.join(BASE_DIR, 'benchmark_results.json'), 'a') as f:
