    ...
    def _fetch_and_process(self, url: str, depth: int) -> None:
        ...
        if resp.status_code in RETRY_STATUS_CODES:
            ...
            if retry_resp is not None:
                ...
                if solve_sg_captcha(self.session, self.base, parsed_url.path):
                    try:
        ...
        # Reset probe 403 streak on any successful probe response
        ...
        if is_sg_captcha_response(resp):
            ...
            if solved:
                # Brief pause to avoid immediate re-challenge
                time.sleep(self.delay * 2)
                try:
                    resp = self.session.get(
                        url, timeout=REQUEST_TIMEOUT, allow_redirects=True,
                ...
                if not resp.ok or is_sg_captcha_response(resp):
                    log.warning("  [SG-CAPTCHA] Still blocked after solve: "
                ...
                log.warning("  [SG-CAPTCHA] Failed to solve for %s", url)
                self._stats["waf"] += 1
                return
