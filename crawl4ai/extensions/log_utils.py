...
Ported from ``web_crawler.utils.log``.  Provides a clean logging system with:
* ANSI colour highlights for ``[CATEGORY]`` tags (works with or without ``colorlog``)
* GitHub Actions CI support (``::warning::``, ``::error::``, ``::group::``)

Fully self-contained with no ``web_crawler`` imports.
"""
...
log = logging.getLogger("crawl4ai.extensions")
