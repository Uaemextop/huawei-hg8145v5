...
Identifies pages that return HTTP 200 but are actually "not found" pages.
Uses three detection layers:

1. **Hash match** – exact SHA-256 fingerprint match against a known-404
   baseline page.
2. **Size + keyword** – body size within ±20 % of baseline *and* contains
   a "not found" keyword.
3. **Title keyword** – ``<title>`` contains ``404``, ``not found``, etc.
"""
